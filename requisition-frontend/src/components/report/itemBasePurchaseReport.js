...
const ItemBasePurchaseReport = forwardRef(
    ({ data, isLoading, columns }, ref) => {
        if (isLoading) {
            return <Loading />
        }
                ...
                <table
                    ...
                    <tbody className={`shadow-none text-gray-800`}>
                        ...
                        <tr>
                            ...
                            <td
                                className={`border p-1 font-bold text-right px-2`}>
                                {Object.keys(data?.purchase ?? [])
                                    ?.reduce((o, k) => {
                                        return data?.purchase[k].reduce(
                                            (o, i) => {
