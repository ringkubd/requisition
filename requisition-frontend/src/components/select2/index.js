...
const Select2 = ({options = [], className, inputCss, value = "", key = Math.floor(Math.random()), ...other}) => {
    ...
    useEffect(() => {
        ...
        ulElementRef.current.addEventListener('scroll', (a) => {
            ...
            if(current.offsetHeight + current.scrollTop > current.scrollHeight){
                ...
                let newfilterOptions = options.filter((e) => regexp.test(e.text)).slice(0,currentScroll * 50);
    ...
    function handleSearch(e){
        ...
        const regexp = new RegExp(`^${value}`, 'i');
        let filterOptions = options.filter((e) => regexp.test(e.text)).slice(0,50)
        setFilterOptions(filterOptions)
    }


    return (
        <div className={`flex flex-col rounded z-50`} ref={containerRef} id={`select_container`} key={key}>
            <div className={`mb-4`} onClick={()=>setOpen(true)}>
                <div
                    className={`w-full border-2 border-gray-300 h-10 rounded-md p-2 ${inputCss} ${open ? 'border-green-500':''}`}
                >{selectedText}</div>
                <ul id={ulId} className={`p-2 border-2 rounded ${open ? 'border-green-500 bg-green-50':'hidden'} max-h-64 overflow-scroll`} ref={ulElementRef}>
                    <li className={`bg-gray-300 space-y-2 p-2 my-1 rounded shadow-2xl relative`}>
                        <input type="text" className={`w-full rounded-md static`} placeholder={`Search`} onChange={handleSearch} ref={searchInputRef}/>
                    </li>
                    {/*<li className={`bg-gray-300 space-y-2 p-2 my-1 rounded hover:cursor-not-allowed shadow-2xl`}>Select an option</li>*/}
                    {
                        filterOptions.length ? filterOptions.map((o, index) => <li
                                key={index}
                                className={`p-2 space-y-2 my-1 bg-gray-50 rounded hover:bg-green-400 hover:cursor-pointer shadow-2xl ${selectedValue === o.value ? 'bg-green-400' : ''}`}
                                value={o.value}
                            >
                                {capitalizeFirstLetter(o.text)}
                        </li>)
                            : <li className={`bg-gray-300 space-y-2 p-2 my-1 rounded hover:cursor-not-allowed shadow-2xl}`}>No option found</li>
                    }
                </ul>
            </div>
            <select
            ...
            >
                ...
                {
                    filterOptions.map((o, index) => <option key={index} value={o.value}>{o.text}</option>)
                }
            </select>
        </div>
    )
