...
export default function Permissions() {
    ...
    const {
        ...
        isError: roleIsError,
    } = useEditRolesQuery(router.query.id, {
        skip: !router.query.id,
    })
    const {
        data: permissions,
        isLoading: permissionsIsLaoding,
    ...
    useEffect(() => {
        if (permissions) {
            const max = Object.keys(permissions.data).map(
                module => permissions.data[module].length,
            )
    ...
    useEffect(() => {
        if (role && selectedPermissions.length && permissions) {
            ...
            Object.keys(permissionsData).map(k => {
                ...
                if (
                    ...
                    selectedPermissions.filter(
                        sp => modulePermission.indexOf(sp) !== -1,
                    ).length
                ) {
                    checkedModule.indexOf(k) === -1
                        ? newPermissionModel.push(k)
                        : ''
                } else {
            ...
            if (newPermissionModel.length) {
                ...
                setCheckedModule(
                    checkedModule.filter(
                        cm => removePermissionModel.indexOf(cm) === -1,
                    ),
                )
            }
        }
            ...
            <div className="md:py-8 md:mx-16 mx-0 md:px-4 sm:px-6 lg:px-8">
                <Card>
                    ...
                    <Table>
                        ...
                        <Table.Body>
                            {!permissionsIsLaoding &&
                            !permissionsIsError &&
                            permissions ? (
                                Object.keys(permissions?.data)?.map((p, i) => (
                                    <Table.Row key={i}>
                                        <Table.HeadCell>
                                            <Label htmlFor={p}>
                                                {p.toUpperCase()}
                                            ...
                                            <Checkbox
                                                ...
                                                value={p}
                                                onChange={e =>
                                                    permissionGroupChange(p)
                                                }
                                                checked={
                                                    checkedModule.indexOf(p) !==
                                                    -1
                                                }
                                            />
                                            {checkedModule[p]}
                                        ...
                                        {permissions?.data[p]?.map(per => (
                                            <Table.Cell key={per.id}>
                                                <div
                                                    className={`space-x-4 flex flex-row items-center align-middle`}>
                                                    <Checkbox
                                                        className={`form-checkbox rounded focus:ring-2`}
                                                        id={per.id}
                                                        checked={
