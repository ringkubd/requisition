    ...
    useEffect(() => {
        if (!isLoading && !isError && data) {
            const obj = JSON.parse(
                JSON.stringify(data?.data?.requisition_products),
            )
            const abcObj = obj?.map(rp => {
                rp['estimated_cost'] =
                    parseFloat(rp.product_option?.unit_price) *
        ...
        const updated = requisitionData.map(rd => {
            if (rd.product_id === values.product_id) {
                return {
                    ...
                    last_purchase_date: values?.option_purchase_history[0]
                        ? moment(
                              values?.option_purchase_history[0]?.created_at,
                          ).format('DD MMM Y')
                        : '',
                    required_quantity: '',
    ...
    const updateOtherItems = (label, value, row) => {
        const newRequisitionData = requisitionData.map(rd => {
            if (
                ...
                row.product_option_id === rd.product_option_id
            ) {
                rd[label] = value
                return rd
            }
    ...
    const tableColumns = [
        {
            ...
            selector: row => {
                return (
                    products.filter(p => p.id === row.product_id)[0]?.title ??
                    row?.product?.title
                )
            },
            sortable: true,
