...
const Edit = props => {
    ...
    const { data, isLoading, isError } = useGetSingleCashRequisitionQuery(
        router.query.id,
        {
    ...
    useEffect(() => {
        if (!isLoading && !isError && data) {
            ...
            const abcObj = obj?.map(rp => {
                rp['cost'] =
                    parseFloat(rp?.unit_price) * parseFloat(rp.required_unit)
                return rp
            })
    ...
    const addItems = (values, pageProps) => {
        values.cost =
            parseFloat(values.required_unit) * parseFloat(values.unit_price)
