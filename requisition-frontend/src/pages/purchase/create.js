...
const create = props => {
    ...
    useEffect(() => {
        if (selectedProductOptionId) {
            const unitPrice = products.filter(
                p =>
                    parseInt(p.product_option_id) ===
                    parseInt(selectedProductOptionId),
            )[0]?.unit_price
            const quantity = products.filter(
                p =>
                    parseInt(p.product_option_id) ===
                    parseInt(selectedProductOptionId),
            )[0]?.quantity_to_be_purchase
            const actual_purchase = products.filter(
                p =>
                    parseInt(p.product_option_id) ===
                    parseInt(selectedProductOptionId),
            )[0]?.actual_purchase
            ...
            formikForm.current.setFieldValue(
                'qty',
                quantity ? (quantity - actual_purchase)?.toString() : 0,
            )
            formikForm.current.setFieldValue(
                'unit_price',
                unitPrice?.toString(),
            ...
            formikForm.current.setFieldValue(
                'total_price',
                (quantity
                    ? (quantity - actual_purchase) * unitPrice
                    : 0
                )?.toString(),
            )
        }
            ...
            options: responseJSON.data.map(r => {
                return {
                    label:
                        r.prf_no +
                        ' (' +
                        moment(r.created_at).format('DD-MMM-Y') +
                        ' by ' +
                        r?.user?.name +
                        ')',
                    value: r.id,
            ...
            <AppLayout
                ...
                <div className="md:py-8 md:mx-16 mx-auto px-4 sm:px-6 lg:px-8">
                    <Card className="min-h-screen">
                        ...
                        <div
                            className={`flex flex-col justify-center justify-items-center items-center basis-2/4 w-full space-y-8`}>
                                                ...
                                                <Select
                                                    ...
                                                    options={products?.map(
                                                        p => ({
                                                            ...
                                                            label:
                                                                p.title +
                                                                (!p.product_option?.option_value?.includes(
                                                                    'N/A',
                                                                )
                                                                    ? ' - ' +
                                                                      p
