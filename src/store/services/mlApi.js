...
export const mlApi = GeneralBaseAPI.injectEndpoints({
    endpoints: (builder) => ({
        ...
        getForecast: builder.query({
            query: ({ product_id, product_option_id, periods = 6, frequency = 'M', include_history = false, force_refresh = false }) => ({
                url: '/ml/forecast',
                method: 'POST',
                body: {
                    product_id,
        ...
        getForecastData: builder.query({
            query: ({ product_id, product_option_id, aggregation = 'monthly' }) => ({
