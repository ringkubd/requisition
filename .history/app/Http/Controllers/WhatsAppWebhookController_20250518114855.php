        ...
        if ($validity) {
            ...
            if ($message->message_type == "button") {
                ...
                if ($stage == 'ceo') {
                    ...
                    $issue = ProductIssue::find($requisition_id);
                    if ($issue->department_status) {
                        $update = $issue->approval_status()->update([
                            'department_status' => $status,
                            'department_approved_at' => now()
                        ]);
                    } else {
                        $update = $issue->approval_status()->updateOrCreate([
                            'department_status' => $status,
                            'department_approved_at' => now()
                        ]);
                    }
                    ...
                    if ($update) {
                        ...
                        foreach ($notifiedUsers as $notifiedUser) {
                            $requisitor = $requisition->user->name;
                            $notifiedUser->notify(new PushNotification(
                                "A product issue has been generated and for your approval.",
                                "$requisitor is generated an issue P.I. NO. $requisition->prf_no against I.R.F. NO. $requisition->irf_no. Please review and approve.",
                                $requisition
                            ));
                            $notifiedUser->notify(new RequisitionStatusNotification($requisition));
                        }
