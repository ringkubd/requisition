                ...
                foreach ($department->users as $user) {
                    if ($type == "cash" && $user->hasPermissionTo('accounts-approval-cash')) {
                        ...
                        $user->notify(new WhatsAppAccountNotification(
                            ...
                            Component::quickReplyButton([$requisition->id . '_' . $requisitorId . '_2_department_cash']),
                            Component::quickReplyButton([$requisition->id . '_' . $requisitorId . '_3_department_cash']),
                            Component::urlButton(["/purchase-requisition/$requisition->id/whatsapp_view?auth_key=" . OneTimeLogin::generate($requisitorId)->auth_key]),
                            $user->mobile_no
                        ));
                        ...
                        $user->notify(new WhatsAppAccountNotification(
                            ...
                            Component::text($requisition->user->name),
                            Component::text($requisition->prf_no),
                            Component::quickReplyButton([$requisition->id . '_' . $requisitorId->id . '_2_department_cash']),
                            Component::quickReplyButton([$requisition->id . '_' . $requisitorId->id . '_3_department_cash']),
                            Component::urlButton(["/cash-requisition/$requisition->id/whatsapp_view?auth_key=" . OneTimeLogin::generate($requisitorId)->auth_key]),
                            $user->mobile_no
                        ));
                    }
                }
