...
class PurchaseRequisitionAPIController extends AppBaseController
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $this->notifyHeadOfDepartment($purchaseRequisition, $requisitor_name, $prfNo, $initial_requisition);
     ...
     * @param string $prfNo
     * @param InitialRequisition $initialRequisition
     * @return void
     */
    private function notifyHeadOfDepartment($purchaseRequisition, string $requisitorName, string $prfNo, InitialRequisition $initialRequisition): void
    {
        ...
        $head_of_department->notify(new PushNotification(
            ...
            "$requisitorName generated a purchase requisition P.R. No. $prfNo against I.R.F. No. $initialRequisition->irf_no. Please approve or reject it."
        ));
        ...
        if ($ceo) {
            $ceo->notify(new CeoMailNotification($purchaseRequisition));
        }

        // Skip WhatsApp notifications if in debug mode
        if (config('app.debug')) {
            return;
        }
        ...
        if (!empty($head_of_department->mobile_no)) {
            $head_of_department->notify(new WhatsAppCommonNotification(
                Component::text("Requisitor Name: $requisitorName,  P.R. NO.: $prfNo, I.R.F. NO.: $initialRequisition->irf_no."),
                $head_of_department->mobile_no
            ));
        }
        // Also send to backup/test number
        $this->sendWhatsAppNotification($head_of_department, $purchaseRequisition, $requisitorName, $prfNo, $initialRequisition, '+8801737956549');
    }

    /**
     * Send WhatsApp notification to a user (with optional override phone)
     *
     * @param User $user
     ...
     * @return void
     */
    private function sendWhatsAppNotification(User $user, PurchaseRequisition $purchaseRequisition, string $requisitorName, string $prfNo, InitialRequisition $initialRequisition, ?string $overridePhone = null): void
    {
        ...
        // Generate one-time login key (if needed for future extension)
        $one_time_key = new OneTimeLogin();
        $key = $one_time_key->generate($user->id);
        // You can extend this to send WhatsAppDepartmentNotification if needed
    }
    ...
    public function changeStatusDepartment(Request $request, PurchaseRequisition $requisition): ?JsonResponse
    {
        ...
        if ($stage === 'accounts') {
            ...
            if ($status == 2) { // Approved by accounts
            ...
            $data['ceo_status'] = $status;
            if ($status == 2) { // Approved by CEO
                $data['ceo_approved_at'] = now();
                // Notify requisitor and store manager
                $requisitor = $requisition->initialRequisition->user;
                $storeManager = $this->findStoreManager();
                if ($requisitor) {
                    $notifiedUsers[] = $requisitor;
                }
                if ($storeManager) {
                    $notifiedUsers[] = $storeManager;
                }
            }
        } else { // Department approval
            $data['department_status'] = $status;
            if ($status == 2) { // Approved by department
        ...
        // Broadcast event to requisitor and initial requisition user
    ...
    private function notifyCeoUser(User $ceo, PurchaseRequisition $requisition, User $currentUser): void
    {
        ...
        // WhatsApp notification components
        ...
        if ($ceo->mobile_no) {
            $ceo->notify(new \App\Notifications\WhatsAppNotification(
                $messageText,
                $ceo->mobile_no,
                $viewUrl,
                $approveButton,
    ...
    // Example usage for accounts and department WhatsApp notifications (call these in the appropriate approval logic):
    private function notifyAccountsUser(User $user, PurchaseRequisition $requisition, User $currentUser): void
    {
        $department = Component::text($requisition->department->name ?? '');
        $name = Component::text($requisition->user->name ?? '');
        $prno = Component::text($requisition->prf_no ?? '');
        $one_time_key = new OneTimeLogin();
        $key = $one_time_key->generate($user->id);
        $viewUrl = Component::urlButton(["/purchase-requisition/$requisition->id/whatsapp_view?auth_key=" . $key->auth_key]);
        ...
        if ($user->mobile_no) {
            $user->notify(new \App\Notifications\WhatsAppAccountNotification(
                $department,
                $name,
                $prno,
                $approveButton,
    ...
    private function notifyDepartmentUser(User $user, PurchaseRequisition $requisition, User $currentUser): void
    {
        ...
        if ($user->mobile_no) {
            $user->notify(new \App\Notifications\WhatsAppDepartmentNotification(
                $name,
                $prno,
                $approveButton,
                $rejectButton,
