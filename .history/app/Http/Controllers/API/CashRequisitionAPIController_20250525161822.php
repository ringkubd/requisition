...
class CashRequisitionAPIController extends AppBaseController
{
    ...
    {
        $latestCashRequisition = CashRequisition::latest()->first();
        if ($latestCashRequisition) {
            $lastPrfNo = (int) substr($latestCashRequisition->prf_no, -6);
            $newPrfNo = str_pad($lastPrfNo + 1, 6, '0', STR_PAD_LEFT);
        } else {
            $newPrfNo = '000001';
        }

        return 'CR' . date('y') . $newPrfNo;
    }
